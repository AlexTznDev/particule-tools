...
}).setLayout({ name: 'permute', type: 'vec4', inputs: [{ name: 'x', type: 'vec4' }] });
...
}).setLayout({ name: 'taylorInvSqrt', type: 'vec4', inputs: [{ name: 'r', type: 'vec4' }] });
...
export const simplexNoise3d = /*#__PURE__*/ Fn(([v_immutable]) => {
    ...
    const p = vec4(
        permute(permute(permute(i.z.add(vec4(0.0, i1.z, i2.z, 1.0))).add(i.y).add(vec4(0.0, i1.y, i2.y, 1.0))).add(i.x).add(vec4(0.0, i1.x, i2.x, 1.0))),
    ).toVar();
...
}).setLayout({ name: 'snoise', type: 'float', inputs: [{ name: 'v', type: 'vec3' }] });
